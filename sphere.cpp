...
Sphere::Sphere(Vertex c, float r) : center{ c }, radius{ r } {}

void Sphere::intersection(const Ray& ray, Hit &hit) {
	Vector ro;
	hit.flag = false;
...
//Generates a random point anywhere on the surface of the sphere
void Sphere::randomSurfacePoint(Vertex& point) {
	do {
		point.x = ((float)rand() / (float)RAND_MAX) - 0.5f;
		point.y = ((float)rand() / (float)RAND_MAX) - 0.5f;
		point.z = ((float)rand() / (float)RAND_MAX) - 0.5f;
	} while (point.x == 0.0f && point.y == 0.0f && point.z == 0.0f);
	float scale = (point.x * point.x) + (point.y * point.y) + (point.z * point.z);
	scale = radius / sqrtf(scale);
	point.x = (point.x * scale) + center.x;
	point.y = (point.y * scale) + center.y;
	point.z = (point.z * scale) + center.z;
}